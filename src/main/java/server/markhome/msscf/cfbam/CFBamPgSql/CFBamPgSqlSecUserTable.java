// Description: Java 11 PostgreSQL Jdbc DbIO implementation for SecUser.
	...
	implements ICFBamSecUserTable
{
		...
		UUID priorSecUserId )
	{
		...
		try {
			Connection cnx = schema.getCnx();
			String sql = "SELECT * FROM " + schema.getLowerDbSchemaName() + ".sp_page_secuser_all( ?, ?, ?, ?, ?, ? )";
		...
		UUID priorSecUserId )
	{
		...
		try {
			Connection cnx = schema.getCnx();
			String sql = "SELECT * FROM " + schema.getLowerDbSchemaName() + ".sp_page_secuser_by_emconfidx( ?, ?, ?, ?, ?" + ", "
					+		"?" + ", "
					+		"?" + " )";
		...
		UUID priorSecUserId )
	{
		...
		try {
			Connection cnx = schema.getCnx();
			String sql = "SELECT * FROM " + schema.getLowerDbSchemaName() + ".sp_page_secuser_by_pwdresetidx( ?, ?, ?, ?, ?" + ", "
					+		"?" + ", "
					+		"?" + " )";
		...
		UUID priorSecUserId )
	{
		...
		try {
			Connection cnx = schema.getCnx();
			String sql = "SELECT * FROM " + schema.getLowerDbSchemaName() + ".sp_page_secuser_by_defdevidx( ?, ?, ?, ?, ?" + ", "
					+		"?" + ", "
					+		"?" + ", "
					+		"?" + " )";
