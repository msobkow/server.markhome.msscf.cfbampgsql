// Description: Java 11 PostgreSQL Jdbc DbIO implementation for SysCluster.
...
/*
 *	CFBamPgSqlSysClusterTable PostgreSQL Jdbc DbIO implementation
 *	for SysCluster.
 */
public class CFBamPgSqlSysClusterTable
	implements ICFBamSysClusterTable
{
	...
	public CFBamPgSqlSysClusterTable( CFBamPgSqlSchema argSchema ) {
		schema = argSchema;
	}
		...
		CFSecSysClusterBuff Buff )
	{
		...
		try {
			...
			String sql =
				"select * from " + schema.getLowerDbSchemaName() + ".sp_create_sysclus( ?, ?, ?, ?, ?, ?" + ", "
				+		"?" + ", "
				+		"?" + " )";
	...
	public String S_sqlSelectSysClusterDistinctClassCode = null;
	...
	public String getSqlSelectSysClusterDistinctClassCode() {
		if( S_sqlSelectSysClusterDistinctClassCode == null ) {
			S_sqlSelectSysClusterDistinctClassCode =
					...
					+	"FROM " + schema.getLowerDbSchemaName() + ".sysclus AS a014 ";
		}
		return( S_sqlSelectSysClusterDistinctClassCode );
	}

	public String S_sqlSelectSysClusterBuff = null;
	...
	public CFSecSysClusterBuff readDerivedByIdIdx( CFSecAuthorization Authorization,
		int SingletonId )
	{
		final String S_ProcName = "CFBamPgSqlSysClusterTable.readDerivedByIdIdx() ";
		CFSecSysClusterBuff buff;
		if( ! schema.isTransactionOpen() ) {
			throw new CFLibUsageException( getClass(),
				S_ProcName,
		...
		CFSecSysClusterPKey PKey )
	{
		...
		try {
			...
			String sql = "SELECT * FROM " + schema.getLowerDbSchemaName() + ".sp_read_sysclus( ?, ?, ?, ?, ?" + ", "
					+		"?" + " )";
		...
		CFSecSysClusterPKey PKey )
	{
		...
		try {
			Connection cnx = schema.getCnx();
			int SingletonId = PKey.getRequiredSingletonId();
			String sql = "SELECT * FROM " + schema.getLowerDbSchemaName() + ".sp_lock_sysclus( ?, ?, ?, ?, ?" + ", "
					+		"?" + " )";
	...
	public CFSecSysClusterBuff[] readAllBuff( CFSecAuthorization Authorization ) {
		...
		try {
			Connection cnx = schema.getCnx();
			String sql = "SELECT * FROM " + schema.getLowerDbSchemaName() + ".sp_read_sysclus_all( ?, ?, ?, ?, ? )";
		...
		int SingletonId )
	{
		...
		try {
			Connection cnx = schema.getCnx();
			String sql = "SELECT * FROM " + schema.getLowerDbSchemaName() + ".sp_read_sysclus_by_ididx( ?, ?, ?, ?, ?" + ", "
					+		"?" + " )";
		...
		long ClusterId )
	{
		...
		try {
			Connection cnx = schema.getCnx();
			String sql = "SELECT * FROM " + schema.getLowerDbSchemaName() + ".sp_read_sysclus_by_clusteridx( ?, ?, ?, ?, ?" + ", "
					+		"?" + " )";
		...
		CFSecSysClusterBuff Buff )
	{
		...
		try {
			...
			Connection cnx = schema.getCnx();
			String sql =
				"select * from " + schema.getLowerDbSchemaName() + ".sp_update_sysclus( ?, ?, ?, ?, ?, ?" + ", "
						+	"?" + ", "
						+	"?" + ", "
						+	"?" + " )";
		...
		int argSingletonId )
	{
		...
		try {
				Connection cnx = schema.getCnx();
				String sql = "SELECT " + schema.getLowerDbSchemaName() + ".sp_delete_sysclus_by_ididx( ?, ?, ?, ?, ?" + ", "
					+		"?" + " ) as DeletedFlag";
		...
		long argClusterId )
	{
		...
		try {
				Connection cnx = schema.getCnx();
				String sql = "SELECT " + schema.getLowerDbSchemaName() + ".sp_delete_sysclus_by_clusteridx( ?, ?, ?, ?, ?" + ", "
					+		"?" + " ) as DeletedFlag";
